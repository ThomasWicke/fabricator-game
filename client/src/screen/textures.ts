...
/** Functional part sprites attached at spec anchors. Wheel has a visible
 *  spoke so rotation reads. */
export function makePartTextures(scene: Phaser.Scene): void {
  if (!scene.textures.exists("part-wheel")) {
    const s = 26;
    const canvas = scene.textures.createCanvas("part-wheel", s, s)!;
    const ctx = canvas.context;
  ...
  if (!scene.textures.exists("part-leg")) {
    const canvas = scene.textures.createCanvas("part-leg", 12, 24)!;
    const ctx = canvas.context;
    ctx.fillStyle = "#23262c";
    ctx.fillRect(3, 0, 6, 18);
    ctx.fillStyle = "#4a4f59";
    ctx.fillRect(1, 17, 10, 6);
    canvas.refresh();
  }
  if (!scene.textures.exists("part-float")) {
    const w = 40;
    const h = 16;
    const canvas = scene.textures.createCanvas("part-float", w, h)!;
    const ctx = canvas.context;
    ctx.fillStyle = "#23262c";
    ctx.beginPath();

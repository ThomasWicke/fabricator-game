...
const ones = clampSpec(structure({ grass: 1, sand: 1, swamp: 1 }));
const zeros = clampSpec(structure({ grass: 0, sand: 0, swamp: 0 }));
check(
...
const stalled = clampSpec({
  ...structure({ grass: 1, sand: 1, swamp: 1 }),
  category: "vehicle",
});
...
const car = computeCost(clampSpec(vehicle("wheels", { grass: 1, sand: 0.8, swamp: 0.15 })));
const tank = computeCost(clampSpec(vehicle("tracks", { grass: 0.85, sand: 0.8, swamp: 0.6 })));
check(
...
const absurd = vehicle("wheels", { grass: 1, sand: 1, swamp: 1 });
...
const raft = computeCost(clampSpec(vehicle("float", { grass: 0.2, sand: 0.3, swamp: 0.9 })));
...
const drillTool = clampSpec({
  ...
  displayName: "Drill",
  size: { w: 36, h: 28 },
  locomotion: { type: "none", speed: 0, terrainModifiers: { grass: 1, sand: 1, swamp: 1 } },
...
const plain = computeCost(clampSpec(vehicle("wheels", { grass: 0.9, sand: 0.8, swamp: 0.1 })));
const harvester = clampSpec(vehicle("wheels", { grass: 0.9, sand: 0.8, swamp: 0.1 }));
...
const glowing = clampSpec(vehicle("wheels", { grass: 0.9, sand: 0.8, swamp: 0.1 }));
